        ...
        <div class="card-toolbar">
            <div class="d-flex flex-wrap align-items-center gap-2">
                ...
                @if((auth()->user()->email ?? '') === 'admin@gmail.com')
                    <button type="button" class="btn btn-light-primary" id="picker_recalculate_today" title="Hitung ulang remaining picker transit untuk hari ini.">Recalculate</button>
                    <button type="button" class="btn btn-light" id="picker_audit_remaining" title="Audit SKU yang masih remaining dan indikasi penyebabnya.">Audit</button>
                @endif
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        recalcBtn?.addEventListener('click', async () => {
            ...
            try {
                ...
                recalcBtn.textContent = oldText || 'Recalculate';
            }
        });
        ...
        tableEl.on('click', '.btn-picker-status', async function() {
            const status = this.getAttribute('data-status') || '';
            const title = this.getAttribute('data-title') || '';
            openStatus(status, title);
        });

        tableEl.on('dblclick', 'tr', async function() {
            // Get date and sku from row data
            const row = dt.row(this).data();
            if (!row || !row.date || !row.sku) return;
            
            if (!pickerTransitDetailModal || !pickerTransitDetailUrl) return;
